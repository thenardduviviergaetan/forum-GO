...
func CreatePost(db *sql.DB, post *models.Post) (int, error) {
	_, err := db.Exec("INSERT INTO post(authorid, author, categoryid, title, content, likes, dislikes, creation) VALUES(?,?,?,?,?,?,?, date())",
		post.AuthorID, post.Author, post.Categoryid, post.Title, post.Content, 0, 0)
	if err != nil {
