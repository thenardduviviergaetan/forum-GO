...
func GetGoogleToken(code, id, secret string) (string, string, error) {
	...
	if errUnmarshal != nil || googleTokenResp == nil || googleTokenResp["error"] != nil {
		fmt.Println("[Github login][Token] -> error trying to unmarshal response body", errUnmarshal)
		return "", "", fmt.Errorf("[Github login][Token] -> error trying to unmarshal response body\n%v\n", errUnmarshal)
	}
